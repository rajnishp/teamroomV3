...
class TeamsMySqlExtDAO extends TeamsMySqlDAO{
	...
	public function queryAllTeamMembers($projectId){
		$sql = "SELECT DISTINCT team.id, user.id as user_id, user.first_name, user.last_name, user.username, user.rank 
	...
	protected function readRowAllTeamMembers($row){
		$team = new TeamMembers($row['user_id'], $row['first_name'],$row['last_name'],$row['rank'], $row['username'], $row['id']);
